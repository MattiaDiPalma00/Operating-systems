...
int random2;
	...
	sem_wait(info->semaforo1);
	random = rand()%info->N;
	while(var <2 && info->finito < 2*info->N){
	random2 = rand()%info->N;
	info->vettore[info->finito] = info->matrice[rigatmp][random2];
	info->finito++;
	var++;
...
info.semaforo3 = sem_open(nomesem3,O_CREAT | O_EXCL,0644,info.N);
